...
export default function Navbar() {
    ...
    const fallbackAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || '')}&background=43552c`;
    const avatarSrc = user?.avatar_url
        ? (user.avatar_url.startsWith('http')
            ? user.avatar_url
            : `${import.meta.env.VITE_API_URL || ''}/storage/${user.avatar_url.replace(/^\/?(storage\/)?/, '')}`)
        : fallbackAvatar;
