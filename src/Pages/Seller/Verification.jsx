...
export default function SellerVerification() {
    ...
    const is_seller = user?.role === 'seller' || user?.is_seller;
    ...
    const fetchVerificationStatus = async () => {
        ...
        try {
            ...
            const verificationData = res.data?.data || null;
            setLatestVerification(verificationData);
            
            // Auto-refresh user role status in AuthContext when approved, avoiding logout or manual reload
            if (verificationData?.status === 'approved' && (!user?.is_seller || user?.role !== 'seller')) {
                const userRes = await api.get('/user');
                if (userRes.data?.data && setUser) {
