...
export default function SellerBalance() {
    ...
    const [bank_name, setBankName] = useState('bca');
    ...
    const handleWithdrawal = async (e) => {
        ...
        if (amountNum < 10000) {
            setError({ amount: 'Minimal penarikan adalah Rp 10.000' });
            return;
        }

        if (amountNum > totalDilepas) {
            setError({ amount: 'Saldo aktif tidak mencukupi.' });
            return;
        }
        ...
        try {
            ...
            setSuccessMessage(res.data.message || 'Penarikan berhasil diproses!');
            setAmount('');
            setAccountNumber('');
            
            // Refresh balance and transactions history
            fetchBalance();
            ...
            if (responseData?.errors) {
                ...
                setError(responseData?.message || 'Gagal memproses penarikan.');
            }
        } finally {
            setIsLoading(false);
        }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-gray-100">
                        ...
                        <form onSubmit={handleWithdrawal} className="p-6">
                            {successMessage && (
                                <div className="mb-4 bg-green-50 border border-green-200 text-green-700 p-3 rounded-lg text-sm font-medium flex items-center gap-2 animate-fade-in">
                            ...
                            {error && typeof error === 'string' && (
                                <div className="mb-4 bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg text-sm font-medium flex items-center gap-2 animate-fade-in">
                            ...
                            <div className="space-y-4 mb-6">
                                <div>
                                    ...
                                    {error?.bank_name && (
                                        <p className="text-red-500 text-sm mt-1">{Array.isArray(error.bank_name) ? error.bank_name[0] : error.bank_name}</p>
                                    )}
                                </div>
                                <div>
                                    ...
                                    {error?.account_number && (
                                        <p className="text-red-500 text-sm mt-1">{Array.isArray(error.account_number) ? error.account_number[0] : error.account_number}</p>
                                    )}
                                </div>
                                <div>
                                    ...
                                    />
                                    {error?.amount && (
                                        <p className="text-red-500 text-sm mt-1">{Array.isArray(error.amount) ? error.amount[0] : error.amount}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <button 
                                type="submit" 
                                disabled={isLoading || !amount || !account_number}
                            ...
                            >
                                ...
                                {isLoading ? 'Memproses...' : 'Tarik Sekarang'}
                            </button>
                        </form>
                    </div>
                </div>
